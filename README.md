...
Installed touch PWAs provide a custom pull-to-refresh gesture only when the page is already at the top. The page content remains fixed while a compact indicator follows the finger with progressively stronger resistance and reveals the 96-pixel release threshold; supported Android/Chromium devices also receive stepped vibration feedback. Releasing past the threshold refreshes the current API-backed public or admin content without navigation or a full page reload, keeps only the indicator visible for the asynchronous update, and temporarily locks touch scrolling until the indicator has finished hiding. Normal downward scrolling resumes immediately afterwards. Offline attempts preserve the current view. WebKit does not expose the Vibration API, so iPhone and iPad use the same visual interaction without haptic feedback.
