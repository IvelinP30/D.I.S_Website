...
Installed touch PWAs provide a custom pull-to-refresh gesture only when the page is already at the top. The content follows the finger through a progressively resistant transform, a compact indicator reveals the 96-pixel release threshold, and supported Android/Chromium devices receive stepped vibration feedback. Releasing past the threshold refreshes the current API-backed public or admin content without navigation or a full page reload, keeps the indicator visible for the asynchronous update, and then returns the content smoothly to rest. Offline attempts preserve the current view. WebKit does not expose the Vibration API, so iPhone and iPad use the same visual interaction without haptic feedback.
